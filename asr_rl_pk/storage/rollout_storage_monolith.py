...
class RolloutStorage_Monolith:
    """Design for storing transitions collected by the MonolithPolicyRunner with distillation_monolith. 
    It supports distillation training types, as well as recurrent policies and RND."""
    class Transition:
        def __init__(self):
            self.observations = None
            self.latent_student_observations = None
            self.privileged_observations = None
            self.latent_teacher_observations = None
            self.actions = None
    ...
    def __init__(
        ...
        if training_type == "distillation":
            ...
            self.latent_student_obs = torch.zeros(num_transitions_per_env, num_envs, *latent_student_obs_shape, device=self.device)
            self.latent_teacher_obs = torch.zeros(num_transitions_per_env, num_envs, *latent_teacher_obs_shape, device=self.device)
    ...
    def add_transitions(self, transition: Transition):
        ...
        if self.training_type == "distillation":
            self.latent_student_obs[self.step].copy_(transition.latent_student_observations)
            self.latent_teacher_obs[self.step].copy_(transition.latent_teacher_observations)
    ...
    def generator(self):
        ...
        for i in range(self.num_transitions_per_env):
            ...
            yield self.observations[i],self.latent_student_obs[i], privileged_observations, self.latent_teacher_obs[i], self.actions[i], self.privileged_actions[
                i
            ], self.dones[i]
