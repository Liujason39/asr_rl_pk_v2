...
class EncoderRolloutStorage:
    ...
    def __init__(
        ...
        if training_type == "VisualEncoderbuild_multihead":
    ...
    def add_transitions(self, transition: Transition):
        ...
        if self.training_type == "VisualEncoderbuild_multihead":
            ...
            ValueError(f"There are no encoder_obs for encoder.")
        # for distillation
    ...
    # for visual encoder training
    def encoder_generator(self):
        if self.training_type != "VisualEncoderbuild_multihead":
            raise ValueError("This function is only available for visual encoder training.")
        ...
        for i in range(self.num_transitions_per_env):
            if self.privileged_observations is not None:
                ...
                privileged_observations = self.observations[i]
            yield self.observations[i], privileged_observations, self.encoder_observations[i], self.actions[i], self.privileged_actions[
                i
            ], self.dones[i]
