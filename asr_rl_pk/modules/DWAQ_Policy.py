...
class dwaq_policy(nn.Module):
    ...
    def prepare_actor_obs(
        ...
        # 1) 更新 history
        proprio_hist = self.proprio_hist_buffer.append(actor_obs)
        ...
        # 5) actor input
        actor_input = torch.cat([vel_for_actor, actor_obs, z_p], dim=-1)
    ...
    def build_actor_obs_from_history(
        ...
        """
        actor_obs:        [B, obs_dim]
        proprio_hist:     [B, H, obs_dim]   # 直接使用 rollout storage 存下來的 history
        true_velocity_obs:[B, vel_dim]
        boot_mask:        [B, 1]            # rollout 當下記錄的 mask
        """

        # 1) 直接用外部 history 做 VAE，不碰 internal buffer
        z_p, mu_p, logvar_p, pred_next_obs = self.forward_proprio_vae(proprio_hist)
        ...
        if inference:
            ...
            if boot_mask is None:
                raise ValueError("boot_mask must be provided during update.")
            used_boot_mask = boot_mask.float()
            vel_for_actor = used_boot_mask * est_vel + (1.0 - used_boot_mask) * true_velocity_obs
