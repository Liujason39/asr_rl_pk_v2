...
class dwaqae_policy(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        layers = []
        in_dim = visual_hidden_dims[-1] # follow with proprio_encoder output dims 
        ...
        # Policy
        actor_layers = []
        actor_layers.append(nn.Linear(num_true_vel_obs+num_actor_obs+proprio_hidden_dims[-1]+visual_hidden_dims[-1], actor_hidden_dims[0]))
    ...
    @torch.no_grad()
    def act_inference_TrueVel(self, obs, visual_obs, true_velocity_obs, return_aux=True):
        """
        Inference-time action generation using only current observation.
        Internal history buffer will be updated automatically.
    ...
    def build_actor_obs_from_history(
        ...
        actor_input = torch.cat([vel_for_actor, actor_obs, z_p, z_e], dim=-1)

        pred_hm = self.hm_decoder(z_e)
        return {
    ...
    def act_for_onnx_transfer(self, 
        ...
        actor_obs = torch.cat([est_v, obs, z_p, z_e], dim=-1)
    ...
    def act_for_onnx_transfer_with_external_vel(
        ...
        **kwargs,
        ...
        actor_obs = torch.cat([vel, obs, z_p, z_e], dim=-1)
        action = self.actor(actor_obs)
