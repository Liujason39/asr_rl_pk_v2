...
from asr_rl_pk.modules import ActorCritic, monolithicpolicy_p1, monolithicpolicy_p3
from asr_rl_pk.modules.rnd import RandomNetworkDistillation
from asr_rl_pk.storage import RolloutStorage
...
class PPO_Monolith:
    ...
    policy: monolithicpolicy_p1 | monolithicpolicy_p3
    ...
    def act(self, obs, latent_obs):
        if self.policy.is_recurrent:
            self.transition.hidden_states = self.policy.get_hidden_states()
        # compute the actions and values
        self.transition.actions = self.policy.act(obs, latent_obs).detach()
        self.transition.values = self.policy.evaluate(obs, latent_obs).detach()
        self.transition.actions_log_prob = self.policy.get_actions_log_prob(self.transition.actions).detach()
        self.transition.action_mean = self.policy.action_mean.detach()
        self.transition.action_sigma = self.policy.action_std.detach()
        # need to record obs and critic_obs before env.step()
        self.transition.observations = obs
        self.transition.privileged_observations = latent_obs
    ...
    def compute_returns(self, last_obs, last_latent_obs):
        # compute value for the last step
        last_values = self.policy.evaluate(last_obs, last_latent_obs).detach()
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            if self.symmetry and self.symmetry["use_data_augmentation"]:
                ...
                if is_recurrent_batch:
                    ...
                    # below had been adjusted for monolith: critic_obs_batch -> unaug_abs_batch
                    latent_obs_batch, _ = data_augmentation_func(
                        obs=latent_obs_batch, actions=None, env=self.symmetry["_env"], obs_type="critic"
                    )
            ...
            # # recurrent batch sanity check (應急測試用)
            # if self.policy.is_recurrent and obs_batch.dim() == 3:
            #     T, B, D = obs_batch.shape
            #     if B == 0:
            #         print(f"[WARN] Skip empty recurrent mini-batch: obs={obs_batch.shape}, latent={latent_obs_batch.shape}")
            #         continue
            #     if masks_batch is not None:
            #         valid_cols = masks_batch.any(dim=0)
            #         print(f"[DEBUG] recurrent batch: T={T}, B={B}, valid={valid_cols.sum().item()}/{B}")
            #         if valid_cols.sum().item() == 0:
            #             print(f"[WARN] Skip all-invalid recurrent mini-batch: masks={masks_batch.shape}")
            #             continue
                    
            self.policy.act(obs_batch, latent_obs_batch, masks=masks_batch, hidden_states=hid_states_batch[0])
            ...
            # -- critic
            value_batch = self.policy.evaluate(obs_batch, latent_obs_batch, masks=masks_batch, hidden_states=hid_states_batch[1])
            ...
            if self.symmetry:
                ...
                if self.policy.is_recurrent:
                    ...
                    # -------------------------
                    mean_actions_batch = self.policy.act_inference(obs_batch.detach().clone(), latent_obs_batch.detach().clone())  # (B_aug, A)
            ...
            if self.auto_encode:
                # print("Using auto-encoding loss for learning...")
                # Auto-encoding loss
                reconstructed_obs = self.policy.autoencode_a(latent_obs_batch)
                ae_a_loss = torch.nn.MSELoss()(reconstructed_obs, latent_obs_batch)
                loss += 0.1*ae_a_loss

                # reconstructed_obs = self.policy.autoencode_c(latent_obs_batch)
                # ae_c_loss = torch.nn.MSELoss()(reconstructed_obs, latent_obs_batch)
                # loss += 0.1*ae_c_loss
            ...
            # -- Auto-encoding loss
            if mean_ae_a_loss is not None:
                mean_ae_a_loss += ae_a_loss.item()
                # mean_ae_c_loss += ae_c_loss.item()
        ...
        # -- Auto-encoding loss
        if self.auto_encode:
            mean_ae_a_loss /= num_updates
            # mean_ae_c_loss /= num_updates
        # -- Clear the storage
        self.storage.clear()
        ...
        if self.auto_encode:
            loss_dict["ae_a"] = mean_ae_a_loss
            # loss_dict["ae_c"] = mean_ae_c_loss
        return loss_dict
